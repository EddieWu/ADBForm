...
HBRUSH CButtonST::CtlColor(CDC* pDC, UINT nCtlColor) 
{
    ...
    // warning C4100: 'pDC' : unreferenced formal parameter
...
{	
    ...
    if ( NULL != pParent )
    {
        HWND hParentWnd = pParent->GetSafeHwnd();
        if ( NULL != hParentWnd )
        {
            ::SendMessage(hParentWnd, WM_BUTTON_CLICKED_NOITFY, (WPARAM)this, (LPARAM)0);
        }
    }
...
{
    // xiaoping.jing Warning Fix:
    // warning C4100: 'bIsDisabled' : unreferenced formal parameter
    // warning C4100: 'bIsPressed' : unreferenced formal parameter
    // warning C4100: 'rpItem' : unreferenced formal parameter
...
DWORD CButtonST::OffsetColor(BYTE byColorIndex, short shOffset, BOOL bRepaint)
{
	...
	{
		if (byRed + shOffset < 0)		shOffsetR = (short)(shOffsetR-byRed);
		if (byGreen + shOffset < 0)		shOffsetG = (short)(shOffsetG-byGreen);
		if (byBlue + shOffset < 0)		shOffsetB = (short)(shOffsetB-byBlue);
